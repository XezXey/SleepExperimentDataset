...
corr_all_states_df = pd.concat([corr_resting_df, corr_sleeping_df, corr_activity_df], ignore_index=True)


# Plotting

# Create folder to store image
path_img = './Visualization_Image/Correlation_1sj/' + subject_folder + '/'
...
# Exporting columns for data summary
path_summary = './Visualization_Image/Correlation_1sj/' + subject_folder + '/' + subject_folder + '_summary' + '/'
# Trying to make directory if it's not exist
if not os.path.exists(os.path.dirname(path_summary)):
    try:
        os.makedirs(os.path.dirname(path_img))
    except OSError as exc: #Guard against race condition
        if exc.errno != errno.EEXIST:
            raise
export_columns = ['subject_no', 'corr_HR_empatica', 'p_score_HR_empatica',
...
corr_resting_df[export_columns].describe().to_excel(path_img + subject_folder + 'resting_summary.xlsx')
corr_sleeping_df[export_columns].describe().to_excel(path_img + subject_folder + 'sleeping_summary.xlsx')
corr_activity_df[export_columns].describe().to_excel(path_img + subject_folder + 'activity_summary.xlsx')
...
axes_corr_all_sj_groupby_window.errorbar(corr_all_states_df.index, corr_all_states_df['mean_HR_emfitqs'], yerr=corr_all_states_df['mae_HR_emfitqs'], fmt = 'x')
...
# 3. Correlation of error estimation(mae) with accelerometer 
# 3.1 By states
fig = plt.figure()
my_dpi = 96
fig = plt.figure(figsize=(1920/my_dpi, 1080/my_dpi))
